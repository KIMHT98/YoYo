...
export default function EventDetail({ navigation }) {
  ...
  const renderItem = ({ item }) => {
    return isWait ? (
      <EventBeforeRegist event={item} onPress={() => clickWaitCard(item)} />
    ) : (
  ...
  return (
    <>
      <Container>
        ...
        <View style={styles.rowContainer}>
          <YoYoText type="subTitle" bold color={MainStyle.colors.main}>{event.totalReceivedAmount.toLocaleString()}</YoYoText>
          <YoYoText type="md">원의 마음을 전해주었어요.</YoYoText>
        </View>
        <SearchBar placeholder="이름을 입력해주세요." />
        <TagList onPress={clickTag} selectedTag={selectedTag} size={64} all />
        <View style={{ position: 'relative', flex: 1 }}>
          <SelectButton leftColor={MainStyle.colors.main} rightColor={MainStyle.colors.main} stateHandler={setIsWait} left="등록 대기" right="등록 완료" />
          <FlatList
      ...
      <Button type="fill" radius={0} onPress={clickBottomButton} width="100%"><YoYoText type="md" bold>{isWait ? "직접 등록" : "마음 받으실 곳"}</YoYoText></Button>
    </>
  );
}
...
const styles = StyleSheet.create({
  ...
  waitText: {
    color: 'white', backgroundColor: 'red', paddingVertical: 1, paddingHorizontal: 7, borderRadius: 100, fontWeight: 'bold', textAlign: 'center', position: 'absolute', top: 11, left: '40%'
  }
});
