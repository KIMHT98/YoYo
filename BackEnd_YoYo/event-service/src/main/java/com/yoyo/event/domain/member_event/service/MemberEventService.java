...
public class MemberEventService {
    ...
    public void createSchedule(Long memberId, Long eventId) {
        Event event = findEventById(eventId);
        if (memberEventRepository.existsByMemberIdAndEventId(memberId, eventId)) {
            throw new EventException(ErrorCode.DUPLICATE_MEMBER_EVENT);
        }

        MemberEvent memberEvent = MemberEventDTO.Request.toEntity(memberId, event);
